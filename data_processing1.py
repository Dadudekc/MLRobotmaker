# data_processing1

import configparser

import pandas as pd
import os
import logging
from ta.trend import SMAIndicator, EMAIndicator, MACD
from ta.momentum import RSIIndicator, ROCIndicator
from ta.volatility import BollingerBands

# Initialize logging
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

# Define format2 folder path
# Read configuration from config.ini
base_path = os.path.dirname(os.path.abspath(__file__))
config_file_path = os.path.join(base_path, 'config.ini')
config = configparser.ConfigParser()
config.read(config_file_path)
format1_path = os.path.join(base_path, config.get('Paths', 'format1processeddata', fallback='csv_files/format1'))

# Function to add features and technical indicators
def add_features_and_indicators(df):
    # Simple Moving Average (SMA) for 10 days
    sma_10 = SMAIndicator(df["c"], window=10)
    df["SMA_10"] = sma_10.sma_indicator()
    
    # Exponential Moving Average (EMA) for 21 days
    ema_10 = EMAIndicator(df["c"], window=21)
    df["EMA_10"] = ema_10.ema_indicator()
    
    # Price Rate of Change (RoC)
    roc = ROCIndicator(df["c"], window=10)
    df["Price_RoC"] = roc.roc()
    
    # Relative Strength Index (RSI)
    rsi = RSIIndicator(df["c"], window=10)
    df["RSI"] = rsi.rsi()
    
    # Moving Average Convergence Divergence (MACD)
    macd = MACD(df["c"])
    df["MACD"] = macd.macd()
    
    # Bollinger Bands
    bollinger = BollingerBands(df["c"])
    df["Bollinger_High"] = bollinger.bollinger_hband()
    df["Bollinger_Low"] = bollinger.bollinger_lband()
    df["Bollinger_Mid"] = bollinger.bollinger_mavg()
    
    return df

# Function to process a single CSV file
def process_csv_file(file_path):
    try:
        # Read the CSV file
        df = pd.read_csv(file_path)
        
        # Add features and indicators
        df = add_features_and_indicators(df)
        
        # Save the enriched DataFrame back to the same CSV
        df.to_csv(file_path, index=False)
        logger.info(f"Processed and saved enriched data to {file_path}")
    except Exception as e:
        logger.error(f"An error occurred while processing {file_path}: {e}")

# Main function
def main():
    for filename in os.listdir(format1_path):
        if filename.endswith(".csv"):
            file_path = os.path.join(format1_path, filename)
            process_csv_file(file_path)

if __name__ == "__main__":
    main()
