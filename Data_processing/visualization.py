...
class ChartCreator:
    def __init__(self, parent, data=None, style='yahoo', mav=(3, 6, 9)):
        ...
        self.new_window.title("Chart Creator")
        self.style = style
        self.mav = mav
    ...
    def create_candlestick_chart(self, title="Candlestick Chart", save_path=None):
        if self.data is not None:
            ...
            # Use the DataFrame self.data for plotting
            self.fig, self.axes = mpf.plot(self.data, **plot_kwargs)
            self.canvas = FigureCanvasTkAgg(self.fig, master=self.new_window)
            self.canvas.draw()
            self.canvas.get_tk_widget().pack(fill="both", expand=True)
        else:
            print("No data available to create a candlestick chart.")


    def create_line_chart(self, x_column='date', y_column='close', title="Line Chart", color='blue'):
        if self.data is not None:
            try:
                ...
                plt.plot(self.data.index, self.data[y_column], label=f'{y_column} Price', color=color)
                plt.xlabel('Date')
                plt.ylabel('Price')
                plt.title(title)
                plt.legend()
            ...
            print("No data available to create a line chart.")
