        ...
        try:
            if file.endswith(".csv"):
                ...
                for scaler_type in ['standard', 'power']:
                    ...
                    print("Column names in the loaded data:")
                    print(data.columns)
                    # Preprocess data
                    ...
                    best_model_tuner = tuner.get_best_models(num_models=1)[0]
                    ...
                    # Evaluate the model
                    mse = evaluate_model(best_model_tuner, X_test, y_test_scaled, y_scaler)

                    # Check if this model is the best model so far
                    if mse < best_mse:
                        best_model = best_model_tuner
                        best_mse = mse

        except Exception as e:
            logging.error(f"Could not process {file}. Error: {e} ❌")

    # Save the best model found during the loop
    if best_model is not None:
        best_model.save(os.path.join(models_directory, 'best_model.h5'))
